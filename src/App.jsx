...
import QuickLinksModal from './components/ui/QuickLinksModal';
...
import ThemeAudioPlayer from './components/effects/ThemeAudioPlayer';
...
const App = () => {
  ...
  useEffect(() => {
        if (user) {
            ...
            const unsubscribe = onSnapshot(docRef, async (docSnap) => {
                if (docSnap.exists()) {
                    ...
                    logger.log("Perfil creado automáticamente.");
                }
            }, (error) => {
                logger.error("Error escuchando el perfil", error);
            });
    ...
    if (isLeader && showDashboard) {
        return <>
            <TeamLeaderView initialTab={targetDashboardTab} onLogout={() => setShowDashboard(false)} currentUserTeam={userProfile.team || 'default'} isAdmin={userProfile.role === 'admin'} />
            <ToastContainer />
        </>;
    }
...
const AuthenticatedApp = ({ user, loading, isLeader, onOpenDashboard, userProfile }) => {
    ...
    const briefing = useDailyBriefing(user);
    const [isBriefingModalOpen, setIsBriefingModalOpen] = useState(false);
    // Recordamos la mayor cantidad de pendientes que vimos hasta ahora en esta
    // sesion. Asi, si el TL agrega un anuncio nuevo MIENTRAS el agente ya tiene
    // la app abierta (sin refrescar), el tablon se vuelve a abrir solo para
    // avisarle - en vez de quedar "gastado" tras la primera evaluacion.
    const maxSeenPendingRef = useRef(-1);
    useEffect(() => {
        if (briefing.loading) return;
        const pending = briefing.pendingCount;
        if (pending > maxSeenPendingRef.current) {
            if (pending > 0) setIsBriefingModalOpen(true);
            maxSeenPendingRef.current = pending;
        }
    }, [briefing.loading, briefing.pendingCount]);
   ...
   // --- CÁLCULO DE TIEMPO TOTAL HOY (Para Luciano) ---
    ...
    const logEvent = async (eventName, data = {}) => {
        try { await addDoc(collection(db, "app_analytics"), { eventName, uid: user.uid, timestamp: Date.now(), data }); } catch (e) { logger.error("Log error", e); }
    };
    ...
    useEffect(() => {
        ...
        }, (error) => logger.warn("Index warning:", error));

        return () => { newsUnsub(); quizUnsub(); progressUnsub(); reportsUnsub(); };
    }, [user]); 

    // --- EFECTO 2: GUARDAR SKIN (CON EXCEPCIÓN INMEDIATA PARA LA TL) ---
    const TL_UID = "QSIxvJOoHzV8KpTr7FORHXqEw1E2"; // Reemplaza por su UID real

    useEffect(() => {
        if (user && themeClasses.name) {
            // Si es la TL, guardamos en Firestore al instante para que no se pierda si cierra la app
            if (user.uid === TL_UID) {
                updateDoc(doc(db, "users", user.uid), { currentSkin: themeClasses.name })
                    .catch((e) => logger.error("Error guardando skin de TL", e));
            } else {
                // Para el resto del equipo, mantenemos el debounce de 2 segundos para ahorrar escrituras
                const timeoutId = setTimeout(() => {
                    updateDoc(doc(db, "users", user.uid), { currentSkin: themeClasses.name }).catch((e) => logger.log("Skin sync skip", e));
                }, 2000);
    ...
    const handleQuizComplete = async (reward) => {
        ...
        try {
            ...
            addCoins(reward);
            showToast(`🎉 ¡FELICIDADES!\nHas completado el módulo.`, 'success');
    ...
    const verifyAvailability = async () => {
        ...
        try {
            // Buscamos el documento actualizado directo de Firebase por las dudas
            const docRef = doc(db, "users", user.uid);
            const docSnap = await getDoc(docRef);
            if (docSnap.exists()) {
                const data = docSnap.data();
                return data.availabilitySubmitted || false;
            }
            return false;
        } catch (e) {
    ...
    const handleClaimWeeklyBonus = () => { 
        ...
        if (lastBonusWeek === currentWeek) { showToast("¡Ya has cobrado!", 'warning'); return; } 
        addCoins(100); 
        setLastBonusWeek(currentWeek); 
        showToast(`🎉 ¡FELICIDADES!\nMeta semanal cumplida. +100 Coins.`, 'success'); 
    };
    ...
    // --- FUNCION DE COMPRA CLASICA (Sin Fragmentos) ---
    // FIX SEGURIDAD/BUG: antes esta funcion escribia "walletBalance" directo en
    // users/{uid}, una ubicacion DISTINTA a donde useWallet/wallet.service guardan
    // el saldo real (users/{uid}/data/wallet). Eso generaba dos fuentes de verdad
    // desincronizadas. Ahora spendCoins() es la UNICA responsable de persistir el
    // saldo, y aqui solo actualizamos el inventario (cosmetico, no es moneda).
    const handleBuyItem = (item) => { 
        if (spendCoins(item.price)) { 
            ...
            showToast("¡Item adquirido correctamente!", 'success');
        } else { 
            showToast("No tienes suficientes Lofi Coins.", 'error'); 
        } 
    };
    ...
    return (
        <>
           ...
           <BackgroundEffectsManager themeClasses={themeClasses} activeEffect={activeEffect} />
            ...
            <EconomyBar coins={lofiCoins || 0} />

            {/* 🔥 2. Campanita de Notificaciones (se mantiene visible por su badge en vivo; el resto de
                 las acciones -Ajustes, Líder, Tienda, etc.- ahora viven en el FloatingMenu de abajo a la derecha) 🔥 */}
            ...
            <FloatingMenu
                ...
                onOpenShop={() => setIsMarketModalOpen(true)}
                ...
                briefingPendingCount={briefing.pendingCount}
                briefingStreak={briefing.streak}
                onOpenQuickLinks={() => setIsQuickLinksModalOpen(true)}
            ...
            <DailyBriefingModal
                ...
                checkedIds={briefing.checkedIds}
                isComplete={briefing.isComplete}
                streak={briefing.streak}
                history={briefing.history}
                toggleItem={briefing.toggleItem}
                onOpenAvailability={() => setIsAvailabilityModalOpen(true)}
            />
                ...
                <div className="relative w-11/12 max-w-xl mt-8">
                   <div className={`w-full rounded-2xl shadow-2xl p-4 sm:p-8 pt-8 sm:pt-12 transition-all duration-500 ${themeClasses.cardBg} min-h-[85vh] relative ${cardGlowClass} ${getBorderClass()} app-card`}>
                        ...
                        <div className="relative z-20 space-y-6 h-full flex flex-col">
                            ...
                            {/* 🔥 BANNER DE EVENTO LIMITADO 🔥 */}
                            {themeClasses.name !== 'outside_the_frame' && (
                                <div className="mx-2 p-4 rounded-xl border border-orange-500/50 bg-gradient-to-r from-orange-900/30 to-indigo-900/30 shadow-[0_0_15px_rgba(255,123,0,0.2)] text-center animate-pulse">
                                    <h3 className="text-orange-400 font-bold uppercase tracking-wider text-sm mb-1">
                                        ⚡ Evento Especial de la Semana ⚡
                                    </h3>
                                    <p className="text-xs text-orange-100/80">
                                        ¡Probá la nueva versión inmersiva del Task Manager! Busca en la tienda y equipá el tema <strong>Outside The Frame</strong> totalmente gratis.
                                    </p>
                                </div>
                            )}

                            {/* 🔥 REPRODUCTOR DENTRO DE LA TARJETA 🔥 */}
                            <ThemeAudioPlayer activeThemeId={themeClasses.name} />
                                ...
                                <div className="flex-1 flex flex-col gap-4 animate-fadeIn">
                                    ...
                                    {tasks.length > 0 && (
                                        <div className="flex items-center justify-between gap-3 px-1 mb-1 animate-fadeIn">
                                            ...
                                            <button onClick={sortTasksByName} className="px-4 py-2 bg-slate-700/50 hover:bg-slate-600 text-slate-300 text-xs font-bold uppercase tracking-wider rounded-full border border-slate-600 hover:border-slate-400 transition-all shadow-sm active:scale-95 flex items-center gap-2"><span>🔤</span> Ordenar por Tarea</button>
