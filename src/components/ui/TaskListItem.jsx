...
const TaskListItem = ({ task, toggleTimer, deleteTask, updateTaskDetails, themeClasses, resetTaskTimer, celebrationThemeName }) => {
    ...
    // --- CÓDIGO DE COLORES POR TIENDA (Basado en sus avatares) ---
    const getShopColorClass = (shopName) => {
        if (!shopName) return 'border-l-gray-600 bg-gray-800/40';
        const normalized = shopName.toLowerCase();
        
        if (normalized.includes('lujosa')) {
            return 'border-l-indigo-500 bg-indigo-950/20 border-indigo-500/30';
        } else if (normalized.includes('clara') || normalized.includes('tierra')) {
            return 'border-l-orange-600 bg-orange-950/20 border-orange-500/30';
        } else if (normalized.includes('divina')) {
            return 'border-l-cyan-400 bg-cyan-950/20 border-cyan-500/30';
        } else {
            return 'border-l-purple-500 bg-purple-950/20 border-purple-500/30';
        }
    };

    // --- ESTILOS ---
    const shopColorStyle = getShopColorClass(task.shop);
    
    const runningBorderClass = task.running 
        ? (celebrationThemeName === 'biancaBday' ? 'border-l-4 border-pink-400 bg-pink-950/30' : 'border-l-4 border-green-500 bg-green-950/20') 
        : `border-l-4 ${shopColorStyle}`;
    ...
    return (
        <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 sm:gap-4 sm:p-4 rounded-xl shadow-md transition-all duration-300 ${themeClasses.itemBg} ${runningBorderClass} border`}>
            ...
            <div className="flex flex-col gap-1 min-w-0 flex-grow">
                ...
                {isEditingShop ? (
                    ...
                    <span 
                        className={`font-bold text-xs sm:text-sm uppercase tracking-wider opacity-90 cursor-pointer hover:text-white transition-colors ${themeClasses.accentText}`} 
            ...
            <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-4 w-full sm:w-auto flex-shrink-0 pt-2 sm:pt-0 border-t border-gray-700/30 sm:border-none mt-2 sm:mt-0">
                ...
                {isEditingTime ? ( 
                    ...
                    <span 
                        className={`font-mono text-xl sm:text-2xl cursor-pointer hover:scale-105 transition-transform tracking-widest ${task.running ? 'text-green-400 font-bold animate-pulse' : themeClasses.primaryText}`} 
