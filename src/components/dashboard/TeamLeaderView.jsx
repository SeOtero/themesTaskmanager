...
import { getNextWeekID, getCurrentWeekID, getTodayID } from '../../utils/helpers';

// --- IMPORTAMOS LOS COMPONENTES ---
import TLDashboardTab from './TLDashboardTab';
import TLPlannerTab from './TLPlannerTab';
import TLNewsTab from './TLNewsTab';
import TLBriefingTab from './TLBriefingTab';
import TLQuickLinksTab from './TLQuickLinksTab';
...
const TeamLeaderView = ({ initialTab = 'dashboard',onLogout, currentUserTeam, isAdmin }) => {
    ...
    // --- Tablon de Anuncios del Dia ("Que tenes que saber hoy") ---
    const [briefingDate, setBriefingDate] = useState(getTodayID());
    const [briefingItems, setBriefingItems] = useState([]);
    const [newBriefingText, setNewBriefingText] = useState("");
    const [newBriefingTarget, setNewBriefingTarget] = useState("all"); // 'all' o el uid de un agente puntual
    const [newBriefingAction, setNewBriefingAction] = useState(null); // null o 'availability' (acceso directo)
...
const ideasUnsub = onSnapshot(qIdeas, (snap) => setPendingIdeas(snap.docs.map(d => ({id: d.id, ...d.data()}))));
    ...
    // --- Tablon de Anuncios: se suscribe al doc de la fecha elegida ---
    useEffect(() => {
        const unsub = onSnapshot(doc(db, "daily_announcements", briefingDate), (snap) => {
            setBriefingItems(snap.exists() ? (snap.data().items || []) : []);
        }, (err) => logger.error("Error cargando el tablon (TL)", err));
        return () => unsub();
    }, [briefingDate]);

    useEffect(() => {
        const unsub = onSnapshot(query(collection(db, "quick_links"), orderBy("createdAt", "desc")), (snap) => {
            setQuickLinks(snap.docs.map(d => ({id: d.id, ...d.data()})));
        }, (err) => logger.error("Error cargando enlaces rapidos (TL)", err));
        return () => unsub();
    }, []);

    useEffect(() => {
        const unsub = onSnapshot(collection(db, "recurring_announcements"), (snap) => {
            setRecurringItems(snap.docs.map(d => ({id: d.id, ...d.data()})));
        }, (err) => logger.error("Error cargando anuncios recurrentes (TL)", err));
        return () => unsub();
    }, []);
   ...
   // --- FUNCIÓN DE CARGA DE DATOS OPTIMIZADA (ZERO-WASTE) ---
    const fetchTeamData = async () => {
        ...
        try {
            ...
            logger.error("Error cargando reportes:", error); 
        } finally {
            setIsLoading(false);
        }
    };
    // ... (Mantén el resto de handlers igual: fetchIdeas, getDateRange, etc.) ...
    const fetchIdeas = async () => { setIdeasLoading(true); try { let q = query(collection(db, "monday_ideas"), orderBy("timestamp", "desc")); const snap = await getDocs(q); let docs = snap.docs.map(d => ({id: d.id, ...d.data()})); if (!isAdmin) docs = docs.filter(d => d.type !== 'dev'); setTeamIdeas(docs); } catch (e) { logger.error(e); } setIdeasLoading(false); };
    ...
    // --- Handlers del Tablon de Anuncios del Dia ---
    const addBriefingItem = async () => {
        if (!newBriefingText.trim()) return;
        const newItem = {
            id: `item_${Date.now()}`,
            text: newBriefingText.trim(),
            // targetUids vacio = para todos los agentes; con un uid = solo para ese agente puntual
            targetUids: newBriefingTarget === 'all' ? [] : [newBriefingTarget],
            // actionType 'availability' = el item muestra un boton "Abrir Disponibilidad"
            actionType: newBriefingAction || null,
        };
        try {
            await setDoc(doc(db, 'daily_announcements', briefingDate), {
                items: [...briefingItems, newItem],
                updatedAt: Date.now(),
            }, { merge: true });
            setNewBriefingText("");
            setNewBriefingAction(null);
        } catch (e) {
            logger.error("Error agregando item al tablon", e);
            showToast("No se pudo agregar el anuncio.", 'error');
        }
    };
    const removeBriefingItem = async (itemId) => {
        try {
            await setDoc(doc(db, 'daily_announcements', briefingDate), {
                items: briefingItems.filter((it) => it.id !== itemId),
                updatedAt: Date.now(),
            }, { merge: true });
        } catch (e) {
            logger.error("Error borrando item del tablon", e);
            showToast("No se pudo borrar el anuncio.", 'error');
        }
    };

    // --- Handlers de Anuncios Recurrentes ---
    const addRecurringItem = async ({ text, target, actionType, recurrence }) => {
        if (!text.trim()) return;
        try {
            await addDoc(collection(db, 'recurring_announcements'), {
                text: text.trim(),
                targetUids: target === 'all' ? [] : [target],
                actionType: actionType || null,
                recurrence,
                createdAt: Date.now(),
            });
        } catch (e) {
            logger.error("Error agregando anuncio recurrente", e);
            showToast("No se pudo agregar el anuncio recurrente.", 'error');
        }
    };
    const removeRecurringItem = async (id) => {
        try {
            await deleteDoc(doc(db, 'recurring_announcements', id));
        } catch (e) {
            logger.error("Error borrando anuncio recurrente", e);
            showToast("No se pudo borrar el anuncio recurrente.", 'error');
        }
    };

    // --- Handlers de Enlaces Rapidos ---
    const addLink = async () => {
        if (!newLinkLabel.trim() || !newLinkUrl.trim()) return;
        let url = newLinkUrl.trim();
        if (!/^https?:\/\//i.test(url)) url = `https://${url}`;
        try {
            await addDoc(collection(db, 'quick_links'), { label: newLinkLabel.trim(), url, createdAt: Date.now() });
            setNewLinkLabel(""); setNewLinkUrl("");
        } catch (e) {
            logger.error("Error agregando enlace rapido", e);
    ...
    const deleteLink = async (id) => {
        try {
            ...
            logger.error("Error borrando enlace rapido", e);
            showToast("No se pudo borrar el enlace.", 'error');
        }
    };
    ...
    const handleSaveAnalysis = async (id, an) => { await updateDoc(doc(db, "monday_ideas", id), { analysis: an }); showToast("Guardado", 'success'); };
    const handleAnalysisChange = (id, f, v) => { setTeamIdeas(prev => prev.map(i => i.id === id ? { ...i, analysis: { ...(i.analysis || {pros:'', cons:''}), [f]: v } } : i)); };
    const generateMondaySummary = () => { const app = teamIdeas.filter(i => i.type === 'monday' && !i.isArchived && i.status === 'approved'); if (app.length === 0) { showToast("Sin ideas aprobadas.", 'success'); return; } const r = getDateRange(); let s = `📋 *RESUMEN*\n📅 ${r.start} - ${r.end}\n\n`; app.forEach((i, idx) => { s += `💡 *#${idx + 1}* (${i.userName})\n"${i.content}"\n\n`; }); setSummaryText(s); setShowSummaryModal(true); };
    const startEditingUser = (u) => { setEditingUserId(u.id); setEditForm({ role: u.role || 'agent', team: u.team || 'default' }); };
    const saveUserChanges = async (uid) => { try { await updateDoc(doc(db, 'users', uid), editForm); setUsersList(prev => prev.map(u => u.id === uid ? { ...u, ...editForm } : u)); setEditingUserId(null); showToast("Guardado", 'success'); } catch (e) { showToast("Error", 'error'); } };
    const handleGivePoints = async (uid) => { const amt = parseInt(pointsToSend); if (!amt) return; await setDoc(doc(db, "users", uid, "data", "wallet"), { val: { value: increment(amt), coins: increment(amt), lofiCoins: increment(amt) } }, { merge: true }); showToast(`Enviados ${amt}`, 'success'); setPointsToSend(0); };
    const handleApproveRequest = async (req) => { try { await setDoc(doc(db, "weekly_schedules", `${req.uid}_${req.weekId}`), { uid: req.uid, userName: req.userName, weekId: req.weekId, schedule: req.proposedSchedule, updatedAt: Date.now() }); await deleteDoc(doc(db, "schedule_requests", req.id)); showToast("✅ Aprobada", 'success'); } catch (e) { showToast("Error", 'error'); } };
        ...
        <div className="min-h-screen w-full bg-slate-950 text-white font-inter animate-fadeIn pb-20 relative overflow-x-hidden selection:bg-indigo-500 selection:text-white">
            ...
            <div className="relative z-10 max-w-7xl mx-auto p-4 md:p-8">
                <div className="flex flex-wrap gap-2 mb-8 p-1 bg-slate-900/50 backdrop-blur rounded-xl border border-white/5 w-fit">
                    {['dashboard', 'planner', 'briefing', 'links', 'news', 'academy', 'ideas', isAdmin ? 'dev' : null, isAdmin ? 'users' : null].filter(Boolean).map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2.5 rounded-lg font-bold text-xs uppercase tracking-wide transition-all ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
                            {{ dashboard: '📊 Reportes', planner: '🧩 Planner', briefing: '📋 Tablón', links: '🔗 Enlaces', news: '📢 Noticias', academy: '🎓 Academy', ideas: '💡 Ideas', dev: '👨‍💻 Dev', users: '👥 Usuarios' }[tab]}
                        </button>
                    ))}
                </div>
                ...
                {activeTab === 'briefing' && <TLBriefingTab briefingDate={briefingDate} setBriefingDate={setBriefingDate} briefingItems={briefingItems} newBriefingText={newBriefingText} setNewBriefingText={setNewBriefingText} newBriefingTarget={newBriefingTarget} setNewBriefingTarget={setNewBriefingTarget} newBriefingAction={newBriefingAction} setNewBriefingAction={setNewBriefingAction} addBriefingItem={addBriefingItem} removeBriefingItem={removeBriefingItem} recurringItems={recurringItems} addRecurringItem={addRecurringItem} removeRecurringItem={removeRecurringItem} agentsList={usersList} todayID={getTodayID()} />}
                {activeTab === 'links' && <TLQuickLinksTab links={quickLinks} newLinkLabel={newLinkLabel} setNewLinkLabel={setNewLinkLabel} newLinkUrl={newLinkUrl} setNewLinkUrl={setNewLinkUrl} addLink={addLink} deleteLink={deleteLink} />}
