...
const TLPlannerTab = ({ 
    ...
    const handleDrop = async (e, taskName, shopName) => {
        ...
        } catch (err) { logger.error(err); showToast("Error de permisos.", 'error'); }
    };
    ...
    const removeAssign = async (taskName, shopName, agentId) => {
        ...
        setWeeklyPlan(newPlan);
        try { await setDoc(doc(db, "weekly_plans", nextWeekId), { plan: newPlan }, { merge: true }); } catch (err) { logger.error(err); }
    };
