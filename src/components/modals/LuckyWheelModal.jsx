...
const LuckyWheelModal = ({ isOpen, onClose, userCoins, setLofiCoins }) => {
    ...
    const spinWheel = () => {
        if (userCoins < SPIN_COST) {
            showToast("No tienes suficientes monedas (Costo: 50). ¡Mira un anuncio para ganar más!", 'error');
            return;
        }
        if (spinning) return;
        ...
        const interval = setInterval(() => {
            setAdProgress(prev => {
                if (prev >= 100) {
                    ...
                    showToast("¡Gracias por ver el anuncio! +50 Monedas recibidas.", 'success');
                    return 100;
                }
