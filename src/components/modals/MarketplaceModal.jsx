                ...
                <div className="px-8 py-6 border-b border-white/5 flex justify-between items-center bg-[#0B0F19]">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-3 bg-white/5 px-4 py-1.5 rounded-full border border-white/10">
                            {/* Sin p-0.5 en el contenedor */}
                            <div className="w-8 h-8 bg-[#d1c4e9] rounded-full flex items-center justify-center overflow-hidden border-2 border-yellow-500/50">
                                {/* Con scale-[1.7] en la imagen */}
                                <img src={myLogo} alt="Lofi Coins" className="w-full h-full object-cover scale-[1.7]" onError={(e) => {e.target.style.display='none'; e.target.nextSibling.style.display='block'}} />
                                <span className="hidden text-lg">💰</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[9px] font-bold text-slate-400 uppercase leading-none"> LOFI COINS</span>
                ...
                <div className="flex-1 overflow-y-auto px-8 pb-8 custom-scrollbar bg-[#0B0F19]">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                       {filteredItems.map((item) => {
                            ...
                            // 🔥 NUEVO: Detectamos el tema gratuito temporal 🔥
                            const isLimitedFree = item.id === 'outside_the_frame';
                            ...
                            if (isSignature) {
                                ...
                                // ✨ ESTILO PARA EL EVENTO GRATIS ✨
                                cardBgClass = "bg-gradient-to-b from-green-900/20 to-[#131b2e]";
                                cardBorderClass = "border-green-500/50";
                                hoverClass = "hover:border-green-400 hover:shadow-[0_0_25px_rgba(34,197,94,0.3)] hover:-translate-y-1";
                            }
                            ...
                            return (
                                <div key={item.id} className={`group relative rounded-2xl p-5 border transition-all flex flex-col h-full ${cardBgClass} ${cardBorderClass} ${hoverClass}`}>
                                    ...
                                    {/* 🔥 BADGE EVENTO GRATIS 🔥 */}
                                    {isLimitedFree && (
                                        <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-600 to-emerald-500 text-white text-[10px] font-black px-4 py-1 rounded-full tracking-widest uppercase shadow-[0_0_15px_rgba(34,197,94,0.6)] border border-green-300 z-20 whitespace-nowrap animate-pulse">
                                            ⚡ Gratis x Tiempo Limitado
                                        </div>
                                    )}
